...
```json
{
  ...
  "index_params": [
    ...
    {"field_name": "source_url", "index_type": "TRIE"},
...
**Notes:**
- `source_url` is nullable (optional metadata field)
- `doc_id` and `source_url` have scalar indexes for efficient filter expressions
...
efficient for up to ~10M tenants. See milvuscraft-multi-tenancy for strategy selection.
...
**Tenant-scoped search — always include the tenant_id filter:**
