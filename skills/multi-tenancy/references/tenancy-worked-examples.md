...
schema.add_field("tenant_id", "VARCHAR", max_length=128, is_partition_key=True)
schema.add_field("chunk_text", "VARCHAR", max_length=2048)
schema.add_field("doc_id", "VARCHAR", max_length=128)
schema.add_field("embedding", "FLOAT_VECTOR", dim=768)
...
client.load_collection("tenant_rag")
```
...
**Critical:** never run a search without `filter="tenant_id == '<value>'"`.
